...
impl ByteRing {
    ...
    pub fn write(&mut self, src: &[u8]) -> usize {
        ...
        if first > 0 {
            self.buf[idx..idx + first].copy_from_slice(&src[..first]);
        }
        if n > first {
            self.buf[..(n - first)].copy_from_slice(&src[first..n]);
        }
        self.head
            .store(head.wrapping_add(n as u32) & self.mask(), Ordering::Release);
        n
    }
    ...
    pub fn read(&mut self, dst: &mut [u8]) -> usize {
        ...
        let first = n.min(self.buf.len() - idx);
        if first > 0 {
            dst[..first].copy_from_slice(&self.buf[idx..idx + first]);
        }
        if n > first {
            dst[first..n].copy_from_slice(&self.buf[..(n - first)]);
        }
        self.tail
            .store(tail.wrapping_add(n as u32) & self.mask(), Ordering::Release);
        n
    }
}
